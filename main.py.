...
async def ask_gemini(question: str) -> str:
    try:
        ...
        response = await asyncio.get_event_loop().run_in_executor(
            ...
            f"Отвечай ярко, с душой, с эмодзи. Разбивай на шаги. В конце дай совет или лайфхак. "
            f"Будь как друг, который помогает разобраться. Вопрос: {question}"
        )
...
async def menu_callback(update: Update, context):
    ...
    if query.data == "menu_subjects":
        ...
        await query.edit_message_text(
            ...
            "💬 *Просто задай вопрос* — я объясню понятно и по шагам.\n\n"
            "🔮 *Удачи на экзаменах!*",
            parse_mode="Markdown"
        )
...
async def subject_callback(update: Update, context):
    ...
    await query.edit_message_text(
        ...
        f"Я объясню понятно, по шагам и с душой.\n\n"
        f"📌 *Попробуй:*\n"
        f"• 'Объясни тему {subject_name}'\n"
        f"• 'Реши задачу по {subject_name}'\n"
        f"• 'Что такое ...?'\n\n"
        f"💬 *Пиши — я помогу!*",
        parse_mode="Markdown"
    )
...
async def handle_text(update: Update, context):
    ...
    if subject not in user_data[user_id]["subjects_stats"]:
        user_data[user_id]["subjects_stats"][subject] = {"total": 0, "correct": 0}
...
def main():
    ...
    print("✨ AI Tutor Pro запущен — умный, живой, с душой")
    app.run_polling()
